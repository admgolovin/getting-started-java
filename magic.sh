...
JENKINS_PASSWORD=Slider256$
echo "Your Jenkins connection link is http://$JENKINSHOST:8080/"
curl -s 'http://Anton:'$JENKINS_PASSWORD'@'$JENKINSHOST':8080/crumbIssuer/api/xml?xpath=concat(//crumbRequestField,":",//crumb)' > crumb.txt
set CRUMB=$(cat crumb.txt)
while ! [ -s $(pwd)/crumb.txt ]
do
echo "Your jenkins pod is ready, please wait while jenkins application will start"
curl 'http://Anton:'$JENKINS_PASSWORD'@'$JENKINSHOST':8080/crumbIssuer/api/xml?xpath=concat(//crumbRequestField,":",//crumb)' > crumb.txt
echo $CRUMB
sleep 1
done
...
curl -H $CRUMB -X POST 'http://Anton:'$JENKINS_PASSWORD'@'$JENKINSHOST':8080/credentials/store/system/domain/_/createCredentials' \
--data-urlencode 'json={
  "": "0",
  "credentials": {
    "scope": "GLOBAL",
